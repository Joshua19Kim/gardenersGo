...
The project currently uses ```gradle```, ```Spring Boot```, ```Thymeleaf```, and ```Simple Java Mail```.
...
- Click on the three lines at the end of the new box that has appeared and add a new environment variable with name SJMP and the value given in the teaching team communications chat on Mattermost
